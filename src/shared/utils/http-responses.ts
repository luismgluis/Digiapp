...
import { AppHttpResponse } from "../models";
 ...
 */
export function Ok(response: Response, body?: unknown): Response {
 ...
 * @param body An optional body that will be sent within the response' body.
 */
export function BadRequest(response: Response, body?: unknown): Response {
...
export function UnAuthenticated(response: Response): Response {
	const body: any = {
		errors: [
			{
				code: AppErrorCo.UnAuthenticated,
